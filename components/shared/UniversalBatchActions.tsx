...
import { X, Layers, MoreVertical } from "lucide-react";
...
export function UniversalBatchActions({
  ...
  return (
    <>
      ...
      <div
        className={cn(
          "fixed bottom-4 left-4 right-4 z-50 lg:hidden animate-fade-in",
          className
        )}
      >
      ...
      <div
        className={cn(
          "hidden lg:flex fixed bottom-8 left-1/2 -translate-x-1/2 z-50 items-center justify-center animate-fade-in",
          className
        )}
      >
        <div className="bg-card/95 backdrop-blur-xl px-2 py-2 pl-4 rounded-2xl shadow-2xl border border-border flex items-center gap-4">
          {/* Counter */}
          ...
          <div className="flex items-center gap-1">
            {primaryActions.map((action) => (
              <Button
                ...
                onClick={action.onClick}
                className={cn(
                  "gap-2 h-9 px-3 text-xs font-medium uppercase tracking-wider rounded-xl transition-all duration-200",
                  action.className
                )}
              >
                <action.icon className="h-4 w-4" />
