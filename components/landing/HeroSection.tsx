  ...
  return (
    <section className="relative overflow-hidden py-8 sm:py-12 md:py-24 lg:py-20 bg-background">
      ...
      <div className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
        ...
        <div className="absolute -top-24 -left-24 h-[360px] w-[360px] rounded-full bg-primary/10 blur-[90px] animate-pulse" />
        {/* bottom-right: secondary soft teal */}
        <div
          className="absolute -bottom-24 -right-24 h-[360px] w-[360px] rounded-full bg-secondary/10 blur-[90px] animate-pulse"
          style={{ animationDelay: "1.2s" }}
        />
        {/* center accent: glacier highlight */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-[500px] w-[500px] rounded-full bg-accent/20 blur-[120px]" />
      </div>
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col items-center text-center">
          ...
          {/* ── Status badge ───────────────────────────────────────── */}
          <div className="mb-8 inline-flex items-center gap-2 rounded-full border border-border bg-card backdrop-blur-sm px-4 py-2 shadow-sm transition-all hover:shadow-md hover:border-secondary">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-center gap-3 mb-16 sm:mb-20 w-full px-4 sm:px-0 sm:w-auto">
            ...
            <Link
              ...
              className="inline-flex items-center justify-center h-12 sm:h-14 px-7 sm:px-9 rounded-full font-bold text-sm sm:text-base
                border-2 border-border bg-card backdrop-blur-sm text-foreground
                hover:border-primary hover:bg-accent hover:text-accent-foreground hover:-translate-y-0.5
          ...
          <div className="flex flex-wrap items-center justify-center gap-2.5 sm:gap-3 md:gap-4">
            {TRUST_INDICATORS.map((item, idx) => (
              <div
                key={idx}
                className="inline-flex items-center gap-2 rounded-full border border-border bg-card backdrop-blur-sm
                  px-3.5 py-2 sm:px-4 sm:py-2.5 shadow-sm
                  hover:shadow-md hover:border-border hover:-translate-y-0.5
                  transition-all duration-200"
              >
                <item.icon className={`h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0 ${item.color}`} aria-hidden="true" />
