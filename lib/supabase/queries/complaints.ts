...
export const complaintsService = {
  ...
  async submitComplaint(
    ...
    try {
      ...
      if (error) {
        ...
        throw new Error(error.message || 'Failed to submit complaint due to a database error.');
      }

      return result as SubmitComplaintResponse;
    } catch (error: any) {
      console.error('Exception in submitComplaint:', error);
      throw new Error(error.message || 'An unexpected error occurred while submitting.');
    }
  },
  ...
  async getComplaintById(id: string): Promise<ComplaintWithRelations | null> {
    try {
      ...
      // 2. Fetch Base Complaint (Using maybeSingle instead of single)
      const { data: complaint, error } = await supabase
        ...
        .eq('id', id)
        .maybeSingle(); // FIX: maybeSingle returns null instead of throwing on 404
      ...
      // 3. Parallel Fetch: Attachments, Comments, History, AND Staff Work Logs (for completion photos)
      const [attachmentsRes, commentsRes, historyRes, workLogsRes] = await Promise.all([
        ...
        // Fetch completion logs to find staff-uploaded proof photos
        // Note: This query will return empty for citizens unless the RLS policy is updated.
        supabase
      ...
      if (complaint.assigned_staff_id) {
        ...
        staff = staffData as StaffProfile | null;
      }
      ...
      // Check if we got data (RLS check passed)
      if (workLogsRes.data) {
        workLogsRes.data.forEach((log: any) => {
            if (log.photo_urls && Array.isArray(log.photo_urls)) {
                log.photo_urls.forEach((url: string, index: number) => {
                    staffProofAttachments.push({
                        id: `proof-${log.staff_id}-${index}`, // Virtual ID
                        complaint_id: id,
                        ...
                        uploaded_by_role: 'staff', // This triggers the "Work Completed" UI logic
                        is_public: true,
         ...
         console.warn("Could not fetch staff work logs. Check RLS policies.", workLogsRes.error);
      }
  ...
  async getUserComplaints(params?: {
    ...
    try {
      ...
      if (params?.search_term) {
        ...
        query = query.or(`title.ilike.%${params.search_term}%,tracking_code.ilike.%${params.search_term}%`);
      }
