...
const MapPicker = dynamic(
    ...
    import("@/app/(protected)/citizen/complaints/_components/form-steps/MapPicker"),
  {
    ssr: false,
    loading: () => (
      <div className="h-full w-full bg-muted animate-pulse rounded-xl flex flex-col items-center justify-center border border-border">
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="relative">
            ...
            {errors.ward_id && (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="mt-1.5 flex items-center gap-1.5 text-xs text-destructive font-medium"
              >
          ...
          <div>
            ...
            <Controller
              ...
              render={({ field }) => (
                <textarea
                  {...field}
                  placeholder="Street name, nearby landmarks..."
                  rows={4}
            ...
            <Controller
              ...
              render={({ field }) => (
                <input
                  ...
                  className="w-full px-4 py-2 rounded-xl border border-border bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50"
                />
              )}
            />
          </div>
        ...
        <div className="flex flex-col relative">
          ...
          <div className="flex-1 min-h-[400px] rounded-xl overflow-hidden border border-border bg-muted relative">
            <MapPicker
              onLocationSelect={handleLocationSelect}
              selectedWard={activeWardNumber}
            />

            <div className="absolute top-3 left-1/2 -translate-x-1/2 bg-background/95 backdrop-blur-sm px-3 py-1.5 rounded-xl border border-border text-xs font-medium text-foreground flex items-center gap-1.5 pointer-events-none shadow-md z-10">
          ...
          {errors.location_point && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="mt-2 bg-destructive/10 border border-destructive/30 p-2.5 rounded-xl"
            >
