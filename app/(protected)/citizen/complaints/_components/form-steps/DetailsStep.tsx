  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-1.5 pb-6 border-b border-border">
        <h2 className="text-2xl font-black text-foreground tracking-tight uppercase">Tell us more</h2>
        <p className="text-sm font-bold text-muted-foreground uppercase tracking-widest opacity-60">
          Please describe the problem and add photos if you have them.
        </p>
      </div>
        ...
        <Controller
          ...
          render={({ field }) => (
            <textarea
              {...field}
              rows={6}
              placeholder="Describe the issue in detail. When did it happen? How is it affecting the area?"
      ...
      {/* Priority */}
      <div className="space-y-3">
        <label className="text-xs font-black uppercase tracking-wider text-foreground px-1">How urgent is this?</label>
        <Controller
          name="priority"
          control={control}
          render={({ field }) => (
            <div className="grid grid-cols-3 gap-4">
              {[
                { value: "low", label: "Low", color: "bg-success-green", border: "border-success-green/20" },
                { value: "medium", label: "Medium", color: "bg-warning-amber", border: "border-warning-amber/20" },
                { value: "high", label: "High", color: "bg-destructive", border: "border-destructive/20" },
              ].map((priority) => {
                ...
                return (
                  ...
                  >
                    {isSelected && (
                      <div className={cn("absolute inset-0 opacity-5 transition-all duration-200", priority.color)} />
                    )}
                    <span className={cn(
      ...
      <div className="space-y-3">
        <label className="text-xs font-black uppercase tracking-wider text-foreground px-1">
          Photos <span className="text-muted-foreground/40 font-bold ml-1">(Optional)</span>
        </label>
        ...
        {/* Preview Grid */}
          {previews.length > 0 && (
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-4 mt-6 animate-fade-in">
              {previews.map((preview, index) => (
                <div
                  key={index}
                  className="relative group aspect-square rounded-xl overflow-hidden border border-border bg-card shadow-xs animate-fade-in"
        ...
        <Controller
          ...
          render={({ field }) => (
            <label className="flex items-center justify-between cursor-pointer">
              <div className="space-y-1">
                <h4 className="text-xs font-black uppercase tracking-widest text-foreground">Send as Anonymous</h4>
                <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest opacity-60">
                  Hide my name from this complaint.
                </p>
              </div>
              <div className="relative">
                ...
                />
                <div className={cn(
                  "w-12 h-6 rounded-full transition-all duration-200 border",
                  ...
                  <div
                    className={cn(
                      ...
                      field.value ? "bg-primary-foreground translate-x-6" : "bg-muted-foreground/40 translate-x-0"
                    )}
                  />
                </div>
              </div>
