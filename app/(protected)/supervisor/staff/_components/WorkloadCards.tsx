    ...
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
      {staffCards.map((staff) => {
        ...
        return (
          <div
            key={staff.staffId}
            className="bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col h-[600px] hover:shadow-md transition-shadow"
          >
            {/* Header */}
            ...
            {/* Assignments List */}
            <div className="flex-1 overflow-y-auto p-2 bg-gray-50/50 space-y-2 custom-scrollbar">
              {staff.assignments.length === 0 ? (
                ...
                staff.assignments.map((assignment) => (
                  <div
                    key={assignment.id}
                    className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm group hover:border-blue-200 transition-colors"
                  >
       ...
       >
         {target.name}
                    ...
                    <p
                      className="text-xs font-medium text-gray-900 truncate mb-2"
                      title={assignment.title}
                    >
                      {assignment.title}
                    </p>

                    <div className="flex justify-between items-center text-[10px] text-gray-500 border-t border-gray-100 pt-2">
