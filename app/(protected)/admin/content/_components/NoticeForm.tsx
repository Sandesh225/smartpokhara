...
const schema = z.object({
  ...
  notice_type: z.enum(['announcement', 'maintenance', 'payment', 'event', 'alert']),
  ...
  const onFormSubmit = async (data: any) => {
     try {
       ...
       const submission: NoticeInput = {
           ...
           // If "all" is selected or if public, ward_id should be null
           ward_id: (data.ward_id === 'all' || isPublic) ? null : data.ward_id,
    ...
    <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-6 max-w-2xl">
       <div className="grid grid-cols-1 gap-4">
          <div className="space-y-2">
             <Label>Title</Label>
             <Input {...register("title")} placeholder="e.g. Road Maintenance Alert" />
             {errors.title && <p className="text-red-500 text-xs">{errors.title.message as string}</p>}
          </div>

          <div className="space-y-2">
             <Label>Excerpt (Short Summary)</Label>
             <Input {...register("excerpt")} placeholder="Brief description for notifications..." />
             {errors.excerpt && <p className="text-red-500 text-xs">{errors.excerpt.message as string}</p>}
          </div>

          <div className="space-y-2">
             <Label>Type</Label>
             <Select onValueChange={(v) => setValue("notice_type", v as any)} defaultValue={initialData?.notice_type || 'announcement'}>
          ...
          <div className="space-y-2">
             <Label>Content</Label>
             <Textarea {...register("content")} className="h-40" placeholder="Full details..." />
             {errors.content && <p className="text-red-500 text-xs">{errors.content.message as string}</p>}
          </div>

          <div className="grid grid-cols-2 gap-6 p-4 border rounded-lg bg-gray-50">
             <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                   <Label>Public Notice</Label>
                   <p className="text-xs text-gray-500">Visible to all citizens</p>
                </div>
                <Switch checked={isPublic} onCheckedChange={(c) => setValue("is_public", c)} />
             </div>
             ...
             <div className="flex items-center justify-between">
                ...
                <Switch checked={watch("is_urgent")} onCheckedChange={(c) => setValue("is_urgent", c)} />
             </div>
          </div>
          ...
          {!isPublic && (
              <div className="space-y-2 animate-in fade-in">
                 ...
                 <Select onValueChange={(v) => setValue("ward_id", v)} defaultValue={initialData?.ward_id || "all"}>
                    <SelectTrigger><SelectValue placeholder="Select Ward" /></SelectTrigger>
                    <SelectContent>
                       <SelectItem value="all">All Wards (Internal)</SelectItem>
                       {wards.map(w => (
                          <SelectItem key={w.id} value={w.id}>Ward {w.ward_number} - {w.name}</SelectItem>
                       ))}
                    </SelectContent>
                 </Select>
              </div>
          ...
          <div className="space-y-2">
             ...
             <Input type="datetime-local" {...register("expires_at")} className="block w-full" />
          </div>
       </div>
